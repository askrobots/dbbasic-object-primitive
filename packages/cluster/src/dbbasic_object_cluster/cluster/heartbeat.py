...
def POST(request):
    ...
    # Update this station's heartbeat (with metrics as JSON in 5th column, version in 6th)
    metrics_json = json.dumps(metrics) if metrics else '{}'
    existing[station_id] = [station_id, host, str(port), str(current_time), metrics_json, version]
